...
export class XaiOAuth extends Service {
  ...
  constructor(ctx, config) {
    ...
    // client-side/custom function tools. This is keyed on the resolved model id
    // (the scoped model-selection listener populates variables.provider/model),
    // so the model can live on the ordinary `xai` route instead of needing a
    // dedicated xai-responses route: whenever that model is selected, strip
    // Harness tool schemas and their matching prompt guidance.
    // The pattern spans every published alias (-latest, -beta-*, -experimental-*).
    const multiAgentModel = /^grok-4\.20-multi-agent/
    ctx.on('system-prompt/assemble', async (_assembly, _context, next) => {
      const assembled = await next()
      if (!multiAgentModel.test(assembled.variables?.model ?? '')) return assembled
      return {
        ...assembled,
  ...
  async fetchUsage() {
    ...
    const [billingResponse, settingsResponse, meResponse] = await Promise.all([
      ...
      fetch(ME_URL, { headers: { accept: 'application/json', 'accept-encoding': 'identity', authorization: `Bearer ${access}` } }),
    ])
    if (!billingResponse.ok) {
      throw new Error(`xAI billing request failed (HTTP ${billingResponse.status})`)
    }
    const billing = await readJson(billingResponse)
    const settings = settingsResponse.ok ? await readJson(settingsResponse) : {}
    const me = meResponse.ok ? await readJson(meResponse) : {}
...
/** Parse a usage response even when the body arrives still gzip- or brotli-compressed. */
async function readJson(response) {
  const buf = Buffer.from(await response.arrayBuffer())
  const encoding = (response.headers.get('content-encoding') || '').toLowerCase()
  const candidates = [buf]
  if (encoding.includes('gzip') || (buf.length >= 2 && buf[0] === 0x1f && buf[1] === 0x8b)) {
    try { candidates.unshift(gunzipSync(buf)) } catch { /* keep raw */ }
  }
  if (encoding.includes('br')) {
    try { candidates.unshift(brotliDecompressSync(buf)) } catch { /* keep raw */ }
  }
  if (!encoding) {
    try { candidates.push(brotliDecompressSync(buf)) } catch { /* not brotli */ }
  }
  let lastError
  for (const body of candidates) {
    try { return JSON.parse(body.toString('utf8')) } catch (error) { lastError = error }
  }
  throw lastError
}
